...
// cambio @author dportoles: Incluyo que en vez de clave utilice el campo clavecif
		...
		rs=stmt.executeQuery("SELECT * FROM carto.usuarios");
		 ...
		 while (!identidad && rs.next()){
				    String usuariobd=rs.getString("USUARIO");
						...
						String clavebd=rs.getString("CLAVECIF");
			...
			if (usuarioIn && identidad) {
				...
				rs = stmt.executeQuery("SELECT * FROM carto.t08_cambios ORDER BY OBJECTID");
				if (rs.next()){
				...
				int datos=stmt.executeUpdate("INSERT INTO carto.t08_cambios (OBJECTID,CMUNINE_P,NOMBRE_P,OBSERVAC_P,XUTM_P,YUTM_P,MOTIVO_CAM,USUARIO,CLAVE,F_PROPUEST,IDENTIDAD_P) VALUES ('"+registroAdd+"','"+cmunine_p+"','"+nombre_p+"','"+observac_p+"',"+xutm_p+","+yutm_p+",'"+motivo+"','"+usuario+"','"+clave+"','"+fecha+"','"+id_p+"')");
			  datos=stmt.executeUpdate("UPDATE carto.t08_cambios SET FUENTE = '"+fuente+"' WHERE OBJECTID = '"+registroAdd+"'");
				datos=stmt.executeUpdate("UPDATE carto.t08_cambios SET NOMBRE = '"+nombre+"' WHERE OBJECTID = '"+registroAdd+"'");
				datos=stmt.executeUpdate("UPDATE carto.t08_cambios SET XUTM = "+xutm+" WHERE OBJECTID = '"+registroAdd+"'");
				datos=stmt.executeUpdate("UPDATE carto.t08_cambios SET YUTM = "+yutm+" WHERE OBJECTID = '"+registroAdd+"'");
				datos=stmt.executeUpdate("UPDATE carto.t08_cambios SET CMUNINE = '"+cmunine+"' WHERE OBJECTID = '"+registroAdd+"'");
				datos=stmt.executeUpdate("UPDATE carto.t08_cambios SET OBSERVACIO = '"+observacio+"' WHERE OBJECTID = '"+registroAdd+"'");
				datos=stmt.executeUpdate("UPDATE carto.t08_cambios SET IDENTIDAD = '"+id+"' WHERE OBJECTID = '"+registroAdd+"'");
				datos=stmt.executeUpdate("UPDATE carto.t08_cambios SET ORGANISMO = '"+organismo+"' WHERE OBJECTID = '"+registroAdd+"'");
			  //rs.close();
			}
		  ...
		  try {
	    	...
	    	out.println("Fallo al cerrar resultados SQL: "+e4.getMessage());
	     	...
	     	out.println("Fallo null4 SQL: "+e4.getMessage());
			}
		}
%>
</body>
